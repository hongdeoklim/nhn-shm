...
export default {
  getHead: () => {
    return [{ name: "현장명" }, { name: "재해형태" }, { name: "소속명" }, { name: "부서명" }, { name: "작성자" }];
  },
  getCol: (index, value, func) => {
    try {
      ...
      case 1:
        return typeof func.getContent(value).stdInfo.재해형태 === "object" ? func.getContent(value).stdInfo.재해형태.join(", ") : func.getContent(value).stdInfo.재해형태; // 재해형태
      case 2:
