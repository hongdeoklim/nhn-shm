...
export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '유도원 및 신호수' }, { name: '소속명' }, { name: '직책' }, { name: '부서명' }, { name: '작성자' }]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).inputA_1 // 작업지휘자
      case 2: return func.getContent(value).inputA_2// 소속명
      case 3: return func.getContent(value).inputA_3// 직책
  ...
  getSearchKeyword: (nav) => {
    ...
    case '유도원 및 신호수': returnData.push('content$body.inputA_1*%[:default_skw]%'); break
    case '소속명': returnData.push('content$body.inputA_2*%[:default_skw]%'); break
    case '직책': returnData.push('content$body.inputA_3*%[:default_skw]%'); break
