...
export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '협의일' }, { name: '도급인(명)' }, { name: '하도급인(명)' }, { name: '소속명' }, { name: '부서명' }]
  },
  getCol: (index, value, func) => {
    try {
      ...
      case 2: return func.comma(func.getContent(value).inputA_4) // 도급인(명)
      case 3: return func.comma(func.getContent(value).inputA_5) // 하도급인(명)
  ...
  getSearchKeyword: (nav) => {
    ...
    case '도급인(명)': returnData.push('content$body.inputA_4*%[:default_skw]%'); break
    case '하도급인(명)': returnData.push('content$body.inputA_5*%[:default_skw]%'); break
