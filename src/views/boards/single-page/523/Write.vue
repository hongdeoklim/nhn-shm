<template>
  <div v-if="!isPrint">
    ...
    <!--    작업 상세내용-->
    <table-risk-appraisal v-for="type in form.selectRiskType" :type="type" :list="form.riskAppraisalObject[type]" :isPrint="isPrint" :isReadonly="board.readonly" @updateList="updateList" />
  ...
  <print523 v-else :form="form" :board="board" />
</template>
...
import Print523 from "@/views/boards/single-page/523/Print";
...
export default {
    ...
    form: {
      ...
      handler(value) {
        ...
        for (const data of this.form.selectRiskType) {
          if (data) {
            value.summary["수시위험성평가_공종명"].push({
              [data]: 1,
            });

            if (this.form.stdInfo.field_name) {
              value.summary["수시위험성평가_현장명_공종명"].push({
  ...
  data() {
    return {
      ...
      riskAppraisalTypes: ["금속공사", "기계설비공사", "방수공사", "도장공사", "수장공사", "전기공사", "조적미장공사", "철거공사", "석재 및 타일공사", "통신공사"],
