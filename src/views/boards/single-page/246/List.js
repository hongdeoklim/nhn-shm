...
export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '작업구분' }, { name: '재해형태' }, { name: '소속명' }, { name: '부서명' }, { name: '작성자' }]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContentStringList(func.getContent(value).typeTitleList) // 작업구분
      case 2: return func.getContentStringList(func.getContent(value).stdInfo.재해형태) // 재해형태
  ...
  getSearchKeyword: (nav) => {
    ...
    case '작업구분': returnData.push('content$body.typeTitleList*%[:default_skw]%'); break
      // case '점검일': returnData.push('content$body.stdInfo."점검일"*%[:default_skw]%'); break
      // case '점검장소': returnData.push('content$body.stdInfo."점검장소"*%[:default_skw]%'); break
