...
export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '화재감시자' }, { name: '소속명' }, { name: '직책' }, { name: '부서명' }, { name: '작성자' }]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).inputA_1 // 화재감시자
  ...
  getSearchKeyword: (nav) => {
    ...
    case '화재감시자': returnData.push('content$body.inputA_1*%[:default_skw]%'); break
