export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '자체평가명' }, { name: '종류' }, { name: '재해형태' }, { name: '참여 인원(명)' }, { name: '소속명' }, { name: '부서명' }, { name: '작성자' }]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).inputA_1 // 자체평가명
  ...
  getSearchKeyword: (nav) => {
    ...
    case '점검대상': returnData.push('content$body.inputA_1*%[:default_skw]%'); break
    case '점검일': returnData.push('content$body.inputA_3*%[:default_skw]%'); break
    case '점검장소': returnData.push('content$body.inputA_7*%[:default_skw]%'); break
