...
export default {
  getHead: () => {
    return [{name: '현장명'}, {name: '회의일'}, {name: '노측 위원(명)'}, {name: '사측 위원(명)'}, {name: '소속명'}, {name: '부서명'}]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).inputA_2 // 회의일
  ...
  getSearchKeyword: (nav) => {
    ...
    case '회의일': returnData.push('content$body.inputA_2*%[:default_skw]%'); break
    case '노축 위원(명)': returnData.push('content$body.inputA_4*%[:default_skw]%'); break
