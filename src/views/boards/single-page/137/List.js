...
export default {
  getHead: () => {
    return [{ name: "현장명" }, { name: "점검대상" }, { name: "소속명" }, { name: "부서명" }, { name: "작성자" }];
  },
  getCol: (index, value, func) => {
    try {
      ...
      case 1:
        return func.getContentTargetList(func.getContent(value).targetInspectionList); // 점검대상
      case 2:
  ...
  getSearchKeyword: (nav) => {
    ...
    case "현장명":
      ...
      break;
    case "점검대상":
      returnData.push("content$body.group1[*].target*%[:default_skw]%");
      break;
    case "소속명":
      returnData.push("content$body.stdInfo.department*%[:default_skw]%");
      break;
    case "부서명":
      returnData.push("content$body.stdInfo.com_name*%[:default_skw]%");
      break;
    case "작성자":
