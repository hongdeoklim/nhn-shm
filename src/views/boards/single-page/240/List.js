export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '교육명' }, { name: '종류' }, { name: '재해형태' }, { name: '참여 인원(명)' }, { name: '소속명' }, { name: '부서명' }, { name: '작성자' }]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).inputA_1 // 교육명
  ...
  getSearchKeyword: (nav) => {
    ...
    case '교육명': returnData.push('content$body.inputA_1*%[:default_skw]%'); break
