...
export default {
  getHead: () => {
    return [{ name: '현장명' }, { name: '관리감독자' }, { name: '소속명' }, { name: '부서명' }, { name: '작성자' }]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).inputE_1
      case 2: return func.getContent(value).inputE_2
  ...
  getSearchKeyword: (nav) => {
    ...
    case '관리감독자': returnData.push('content$body.inputE_1*%[:default_skw]%'); break
    case '소속명': returnData.push('content$body.inputE_2*%[:default_skw]%'); break
