export default {
  getHead: () => {
    return [{name: '현장명'}, {name: '점검일'}, {name: '재해형태'}, {name: '소속명'}, {name: '부서명'}]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).stdInfo.field_name // 현장명
      case 1: return func.getContent(value).stdInfo.점검일 // 점검일
      case 2: return func.getContent(value).stdInfo.재해형태 // 재해형태
