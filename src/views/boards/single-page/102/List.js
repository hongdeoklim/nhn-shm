export default {
  getHead: () => {
    return [{name: '대상 현장명'}, {name: '피평가자'}, {name: '평가일'}, {name: '소속명'}, {name: '부서명'}, {name: '작성자'}]
  },
  getCol: (index, value, func) => {
    try {
      switch (index) {
      case 0: return func.getContent(value).companyName // 대상 현장명
      case 1: return func.getContent(value).inputB_4 // 피평가자
      case 2: return func.getContent(value).inputB_5 // 평가일
  ...
  getSearchKeyword: (nav) => {
    ...
    case '피평가자': returnData.push('content$body.inputB_4*%[:default_skw]%'); break
