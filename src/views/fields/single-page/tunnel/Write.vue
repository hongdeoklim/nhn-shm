  ...
  <vx-card>
    <div>
      <div class="flex justify-between items-center">
        <p style="font-size: 18px; font-weight: 700">터널</p>
    ...
    <div class="mt-28px">
      <h5 class="mb-4"><i class="dot"></i> 상세제원(터널)</h5>
      <div class="writeGridContainer">
        <div class="writeGridCol2">
          <div>터널 시점위치 (읍면동리까지 기입)</div>
          <ml-input v-model="form.터널시점위치" placeholder="(시,도) (시,군,구) (읍,면,동) (리, 번지 등 주소)" />
        </div>
        <div class="hidden xl:block"></div>
        <div class="writeGridCol2">
          <div>터널 종점위치 (읍면동리까지 기입)</div>
          <ml-input v-model="form.터널종점위치" placeholder="(시,도) (시,군,구) (읍,면,동) (리, 번지 등 주소)" />
        </div>
        <div class="hidden xl:block"></div>
      </div>
    </div>
    ...
    <div class="mt-28px">
      <table>
        ...
        <tbody>
        <tr>
          <td><ml-input v-model="form.폭_보도" class="w-full" /></td>
          <td><ml-input v-model="form.폭_차도" class="w-full" /></td>
          <td><ml-input v-model="form.계" class="w-full" /></td>
          <td><ml-input v-model="form.연장" class="w-full" /></td>
          <td><ml-input v-model="form.높이" class="w-full" /></td>
        </tr>
        </tbody>
      </table>
    </div>
    ...
    <div style="margin-top: 14px;">
      <table>
        ...
        <tr>
          <th>대표</th>
          <td><ml-input v-model="form.형상_대표" class="w-full" /></td>
          <td><ml-input v-model="form.주요공법_대표" class="w-full" /></td>
          <td><ml-input v-model="form.보조공법_대표" class="w-full" /></td>
          <td><ml-input v-model="form.배수형식_대표" class="w-full" /></td>
          <td><ml-input v-model="form.갱문형식_대표" class="w-full" /></td>
        </tr>
        <tr>
          <th>기타</th>
          <td><ml-input v-model="form.형상_기타" class="w-full" /></td>
          <td><ml-input v-model="form.주요공법_기타" class="w-full" /></td>
          <td><ml-input v-model="form.보조공법_기타" class="w-full" /></td>
          <td><ml-input v-model="form.배수형식_기타" class="w-full" /></td>
          <td><ml-input v-model="form.갱문형식_기타" class="w-full" /></td>
        </tr>
        </tbody>
      </table>
    </div>
    ...
    <div style="margin-top: 14px;">
      <table>
        ...
        <tbody>
        <tr>
          <td><ml-input v-model="form.구조형식" class="w-full" /></td>
          <td>
            <div class="w-full" style="gap: 14px; display: flex; justify-content: center;">
              <vs-radio v-for="option in ['유', '무']" :key="`덕트슬래브_${option}`" v-model="form.덕트슬래브" vs-name="덕트슬래브" :vs-value="option">{{ option }}</vs-radio>
            </div>
          </td>
          <td>
            <div class="w-full" style="gap: 14px; display: flex; justify-content: center;">
              <vs-radio v-for="option in ['유', '무']" :key="`인버트설치_${option}`" v-model="form.인버트설치" vs-name="인버트설치" :vs-value="option">{{ option }}</vs-radio>
            </div>
          </td>
          <td><ml-input v-model="form.차로수_상행" type="number" :min-value="0" class="w-full" /></td>
          <td><ml-input v-model="form.차로수_하행" type="number" :min-value="0" class="w-full" /></td>
          <td><ml-input class="w-full" :value="parseInt(form.차로수_상행 ? form.차로수_상행 : 0) + parseInt(form.차로수_하행 ? form.차로수_하행 : 0)" /></td>
          <td><ml-input v-model="form.종단기울기" class="w-full" /></td>
          <td><ml-input v-model="form.평면선형곡선반경R" class="w-full" /></td>
        </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="mt-28px">
      ...
      <table>
        ...
        <tbody>
        <tr>
          <td><ml-input v-model="form.자연환기1" class="w-full" /></td>
          <td><ml-input v-model="form.강제환기_종류식1" class="w-full" /></td>
          <td><ml-input v-model="form.강제환기_횡류식1" class="w-full" /></td>
          <td><ml-input v-model="form.강제환기_반횡류식1" class="w-full" /></td>
        </tr>
        <tr>
          <td><ml-input v-model="form.자연환기2" class="w-full" /></td>
          <td><ml-input v-model="form.강제환기_종류식2" class="w-full" /></td>
          <td><ml-input v-model="form.강제환기_횡류식2" class="w-full" /></td>
          <td><ml-input v-model="form.강제환기_반횡류식2" class="w-full" /></td>
        </tr>
        <tr>
          <th>조명</th>
          <th>전기</th>
        ...
        <tr>
          <td>
            <div class="w-full" style="gap: 14px; display: flex; justify-content: center;">
              <vs-radio v-for="option in ['유', '무']" :key="`조명_${option}`" v-model="form.부속설비_조명" vs-name="부속설비_조명" :vs-value="option">{{ option }}</vs-radio>
            </div>
          </td>
          <td>
            <div class="w-full" style="gap: 14px; display: flex; justify-content: center;">
              <vs-radio v-for="option in ['유', '무']" :key="`전기_${option}`" v-model="form.부속설비_전기" vs-name="부속설비_전기" :vs-value="option">{{ option }}</vs-radio>
            </div>
          </td>
          <td><ml-input v-model="form.부속설비_소방설비" class="w-full" /></td>
          <td><ml-input v-model="form.부속설비_기타" class="w-full" /></td>
        </tr>
        </tbody>
      </table>
    </div>
    ...
    <div style="margin-top: 14px;">
      <table>
        ...
        <tr>
          <td>
            <file-uploader type="only-image" class="upload-img" v-model="form.표준단면도_1" />
          </td>
          <td>
            <file-uploader type="only-image" class="upload-img" v-model="form.표준단면도_2" />
          </td>
          <td>
            <file-uploader type="only-image" class="upload-img" v-model="form.표준단면도_3" />
          </td>
        </tr>
        </tbody>
      </table>
    ...
    <!-- - 구간(station) 정보 -->
    <div class="mt-28px">
      <h5 class="bold mb-4">- 구간(station) 정보</h5>
      <table-custom
        :controls="true"
        :multiple="true"
        ...
        title=""
        :tableData="[
        { id: '시점', title: '시점 Station No', type: 'text', placeholder: '' },
        { id: '종점', title: '종점 Station No', type: 'text', placeholder: '' },
        { id: '표준패턴', title: '표준패턴 No', type: 'text', placeholder: '' },
      ]"
        :mobile="['target', 'name']"
        v-model="form.구간정보"
        class="mt-4"
  ...
  data() {
    return {
      form: {
        ...
        accessibleVentCover: "",
        designPeriod: { sDate: "", eDate: "" },
        designer: "",
        defectLiabilityExpirationDate2: "",
        constructionPeriod: { sDate: "", eDate: "" },
        contractor: "",
