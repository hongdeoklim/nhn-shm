    ...
    <vx-card>
      <div>
        <div class="flex justify-between items-center">
          <p style="font-size: 18px; font-weight: 700">{{ label }}</p>
        ...
        <div class="mt-28px">
          ...
          <div class="writeGridContainer">
            <div>
              ...
              <div class="radio">
                <vs-radio v-for="option in ['유', '무']" :key="`환기구덮개_${option}`" v-model="form.accessibleVentCover" vs-name="환기구 덮개 (접근 가능한 환기구)" :vs-value="option">{{ option }}</vs-radio>
              </div>
            </div>
          </div>
          <div class="writeGridContainer">
      ...
      <div class="mt-28px">
        <h5 class="mb-4"><i class="dot"></i> 상세제원{{ label ? `(${label})` : '' }}</h5>
        <div class="writeGridContainer">
          <div>
            <div>주용도</div>
...
export default {
  ...
  methods: {
    async loadField() {
      ...
      const facility_category = this.form.facility_category || info.facility_category;
      if (facility_category) {
        this.cat1 = facility_category.cat1;
        this.cat2 = facility_category.cat2;
        this.cat3 = facility_category.cat3;
        this.cat4 = facility_category.classification;
        this.form.facility_category_id = facility_category.id;
      }
    ...
    async onChangeCat3(data) {
      if (data && !this.catLock) {
        ...
        if (this.cat4List.length > 0) {
          const facility_category = this.cat4List[0];
          if (!facility_category.classification) {
            this.form.facility_category_id = facility_category.id;
          }
        }
      }
    ...
    onClickSave() {
      ...
      const option = {
        ...
        facility_category_id: this.cat4 || this.form.facility_category_id, // Category
