  ...
  <vs-row class="px-3 xl:px-0">
    ...
    <vs-col class="w-1/2 pr-2 pb-4">
      ...
      >
        <template v-if="projectId" slot="tools" :set="lv = getLevel">
          ...
          <vs-button class="ml-2" type="border" color="secondary" @click="dataTypeChangeForGraph2('detail')">대상공정</vs-button>
        </template>
      </monthly-comparison-card>
    </vs-col>
    ...
    <vs-col class="w-1/2 pl-2 pb-4">
      <daily-comparison-card
        :title="graph3.title"
        :unit="graph3.unit"
        :begin-date="graph3.beginDate"
        :end-date="graph3.endDate"
        :list="graph3.list"

        @change="dateRangeChangeForGraph3"
      >
        <template v-if="projectId" slot="tools" :set="lv = getLevel">
          ...
          <vs-button class="ml-2" type="border" color="secondary" @click="dataTypeChangeForGraph3('detail')">대상공정</vs-button>
        </template>
      </daily-comparison-card>
    </vs-col>
  </vs-row>
...
export default {
  ...
  methods: {
    ...
    async loadGraph2 () {
      ...
      let summaryKey1 = ['종사자의견청취_공정명', 'count']
      if (dataType === 'field') {
        summaryKey1 = ['종사자의견청취_현장명', 'count']
      }
