      ...
      <vs-col>
        <monthly-comparison-card :title="graph2.title" :year="graph2.year" :month="graph2.month" :unit="graph2.unit" :heads="graph2.heads" :list="graph2.list" :getCol="getColForGraph2" @change="dateChangeForGraph2">
          <template v-if="projectId" slot="tools" :set="(lv = getLevel)">
            ...
            <vs-button class="py-3 px-0 lg:px-8" type="border" color="secondary" @click="dataTypeChangeForGraph2('detail')">예산항목</vs-button>
          </template>
      ...
      <vs-col>
        <daily-comparison-card :title="graph3.title" :unit="graph3.unit" :begin-date="graph3.beginDate" :end-date="graph3.endDate" :list="graph3.list" @change="dateRangeChangeForGraph3">
          <template v-if="projectId" slot="tools" :set="(lv = getLevel)">
            ...
            <vs-button class="py-3 px-0 lg:px-8" type="border" color="secondary" @click="dataTypeChangeForGraph3('detail')">예산항목</vs-button>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      graph2: {
        ...
        unit: "",
        heads: [{ name: "No." }, { name: "분야명" }, { name: "예산" }, { name: "전월대비 증감" }],
        list: [],
  ...
  methods: {
    ...
    async loadGraph2() {
      ...
      let summaryKey = ["안전보건예산_항목명", "sum"];
      if (dataType === "field") {
        summaryKey = ["안전보건예산_현장명", "sum"];
      }
      ...
      if (dataType === "company") {
        this.graph2.heads = [{ name: "No." }, { name: "관련부서" }, { name: "건(원)" }, { name: "전월대비 증감" }];
        ...
        this.graph2.heads = [{ name: "No." }, { name: "현장명" }, { name: "건(원)" }, { name: "전월대비 증감" }];
      } else if (dataType === "detail") {
        this.graph2.heads = [{ name: "No." }, { name: "예산항목" }, { name: "건(원)" }, { name: "전월대비 증감" }];
      }

      this.graph2.list = MonthlyComparisonUtil.moduleWithData(value).map((_data) => {
        return { ..._data, value: this.comma(`${_data.value}`), gap: this.comma(`${_data.gap}`) };
      });
    },
    ...
    getColForGraph2(index, value) {
      ...
      if (index === 3) {
        let updown = "default";
        const gap = this.removeComma(value.gap);
        if (gap > 0) updown = "up";
        else if (gap < 0) updown = "down";
