<template>
  <vs-tabs :value="1">
    ...
    <vs-tab label="담당 현장 정보" @click="onClickTab(1)">
      <div>
        <vx-card class="mb-8">
              ...
              >
                ...
                <template slot-scope="{ data }">
                  <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td> {{ data[indextr].facility_category.cat2 }}</vs-td>
                    <vs-td> {{ data[indextr].address }}</vs-td>
                    <vs-td> {{ data[indextr].be_completed_at }}</vs-td>
                    <vs-td> {{ data[indextr].deposit_amount ? new Intl.NumberFormat().format(parseInt(data[indextr].deposit_amount.replace(/,/g, ''))) : '-' }}</vs-td>
                  </vs-tr>
                </template>
              </vs-table>
            </div>
              ...
              <div class="mt-5">
                <div class="w-full grid" style="grid-template-columns: 1fr 1fr 1fr; column-gap: 1.5rem;">
                  ...
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    ...
                    <comma-input class="w-full flex-1" :readonly="true" v-model="selectedField.deposit_amount"></comma-input>
                  </div>
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">계약번호</div>
                    <ml-input class="w-full flex-1" :readonly="true" v-model="selectedField.contract_code"/>
                  </div>


                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">구분</div>
                    <ml-input class="w-full flex-1" :readonly="true" :value="selectedField && selectedField.facility_category && selectedField.facility_category.cat1 ? selectedField.facility_category.cat1 : ''"/>
                  </div>
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">종류</div>
                    <ml-input class="w-full flex-1" :readonly="true" :value="selectedField && selectedField.facility_category && selectedField.facility_category.cat2 ? selectedField.facility_category.cat2 : ''"/>
                  </div>
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">세분류</div>
                    <ml-input class="w-full flex-1" :readonly="true" :value="selectedField && selectedField.facility_category && selectedField.facility_category.cat3 ? selectedField.facility_category.cat3 : ''"/>
                  </div>
                  ...
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    ...
                    <ml-input class="w-full flex-1" :readonly="true" v-model="selectedField.director_name"/>
                  </div>
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">안전관리자</div>
                    <ml-input class="w-full flex-1" :readonly="true" v-model="selectedField.safety_manager_name"/>
                  </div>


                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">대표번호</div>
                    <ml-input class="w-full flex-1" :readonly="true" v-model="selectedField.phone"/>
                  </div>
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
                    <div class="label-text mr-4">착공일</div>
                    <date-selector class="popup-fixed" :readonly="true" v-model="selectedField.start_work_at"></date-selector>
                  </div>
                  <div class="mb-4" style="display: grid; grid-template-columns: 116px 1fr; align-items: center;">
...
export default {
  ...
  methods: {
    initBreadcrumb () {
      this.$store.commit('menu/UPDATE_PAGE_LIST', [{title: 'Home'}, {title: '현장 현황'}, {title: '현장 현황', active: true}])
    },
    loadCompanyInfo () {
      this.$store.dispatch('company/LOAD_COMPANY', {companyId: this.selectedCompanyId}).then(value => {
        ...
        if (this.companyInfo.id) {
          ...
          this.$store.dispatch('company/LOAD_COMPANY_FIELD', {companyId, projectId, fieldType: '현장(도급발주)'}).then(value => {
