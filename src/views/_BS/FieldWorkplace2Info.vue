                ...
                <div class="grid grid-cols-12 gap-x-6 gap-y-4">
                  ...
                  <div class="col-span-6 flex items-center">
                    <search-nav-start-end
                      class="input-se"
                      label="사업기간"
                      :value="{'sDate': selectedField.work_begin_at, 'eDate': selectedField.work_end_at}"
                      :readonly="true"
                    />
                  </div>
                  <div class="col-span-6 flex items-center">
    ...
    loadCompanyInfo () {
      this.$store.dispatch('company/LOAD_COMPANY', {companyId: this.selectedCompanyId}).then(value => {
        ...
        if (this.companyInfo.id) {
          ...
          this.$store.dispatch('company/LOAD_COMPANY_FIELD', {companyId, projectId, fieldType: '현장(용역위탁)'}).then(value => {
