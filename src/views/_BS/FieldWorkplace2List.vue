...
export default {
  ...
  methods: {
    ...
    async loadFieldList() {
      ...
      const options = { projectId, type: "현장(용역위탁)", showAll: 0 }
    ...
    onClickNew() {
      this.$router.push({ name: "field-workplace2-create" });
    },
    onClickClose() {
    ...
    handleItemClickDetail(data) {
      this.$router.push({ name: "field-workplace2-update", params: { proj_id: this.$route.params.proj_id, postId: data.id } });
    },
