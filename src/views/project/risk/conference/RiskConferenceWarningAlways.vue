...
export default {
  name: 'RiskConferenceWarningAlways',
  components: {
    KPagination,
  ...
  methods: {
    ...
    async loadPostList () {
      ...
      const boardSlug = 'risk_evaluation_association_always' // risk_evaluation_association_always(id:8)
    ...
    handleItemClickDetail (data) {
      this.$router.push({path: `${this.rootPath}/risk/conference/warning-always/${data.post_id}`})
    },
    ...
    handleClickRegister () {
      this.$emit('handleClickRegister', 'warning-always')
    },
