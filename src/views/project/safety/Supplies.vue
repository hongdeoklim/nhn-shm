<template>
  <div class="w-full">
    ...
    <search-nav @handleSearch="handleSearch">
      <search-nav-item-select label="대상기간" v-model="nav.year" :list="nav.yearList" />this.form.evalList
      ...
      <vs-col>
        <vs-table stripe
          ...
          <template slot="thead">
            ...
            <vs-th sort-key="post_title" class="w-1/12"><span class="w-full text-center">제목</span></vs-th>
            <vs-th sort-key="content$preview.title" class="w-1/12"><span class="w-full text-center">작성자</span></vs-th>
...
export default {
    ...
    excelList () {
      ...
      for (const data of this.listAll) {
        list.push({
          ...
          '작성자': this.getContents(data).writer,
          '작성일': data.post.created_at,
  ...
  methods: {
    ...
    handleItemClickDetail (data) {
      this.$router.push({path: `${this.rootPath}/safety/supplies/${data.post_id}`})
    },
