...
export default {
  ...
  components: {
    ...
    KPagination,
    PopupApprovalProcess, PopupExcelDownload, SearchNavItemDate, SearchNavItemKeyword, SearchNav},
  data () {
    return {
      // 검색 네비게이션
      nav: {
  ...
  methods: {
    ...
    handleItemClickDetail (data) {
      if (data.post) {
        this.$router.push(`/project/${data.post.project_id}/risk/assessment/regular/${data.post.id}`)
      }
    },
    handleItemClickDetailBlank (data) {
      if (data.post) {
        const routeData = this.$router.resolve(`/project/${data.post.project_id}/risk/assessment/regular/${data.post.id}`)
