...
export default {
  ...
  methods: {
    ...
    async loadPostList () {
      ...
      const boardSlug = 'risk_evaluation_association' // first_risk(id:6)
    ...
    handleItemClickDetail (data) {
      if (data.post) {
        this.$router.push(`/project/${data.post.project_id}/risk/conference/warning/${data.post.id}`)
      }
    },
    handleItemClickDetailBlank (data) {
      if (data.post) {
        const routeData = this.$router.resolve(`/project/${data.post.project_id}/risk/conference/warning/${data.post.id}`)
