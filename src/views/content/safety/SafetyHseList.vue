    ...
    <vs-row>
      <vs-col>
        ...
        >
          ...
          <template slot="thead">
            <vs-th sort-key="id" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.No')}}</span></vs-th>
            <vs-th sort-key="post_title" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.현장명')}}</span></vs-th>
            <vs-th sort-key="content$company" ><span class="w-full text-center">소속</span></vs-th>
            <vs-th sort-key="content$hse_type_safety" ><span class="w-full text-center">교육구분</span></vs-th>
            <vs-th sort-key="content$preview.work" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.교육명')}}</span></vs-th>
            <vs-th sort-key="content$preview.target" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.교육대상')}}</span></vs-th>
            <vs-th sort-key="content$preview.date" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.교육일')}}</span></vs-th>
            <vs-th sort-key="content$preview.main_manager" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.교육담당')}}</span></vs-th>
            <vs-th sort-key="content$preview.total" ><span class="w-full text-center">{{$t('content.safety.hse.list.column.총인원')}}</span></vs-th>
            <vs-th ><span class="w-full text-center">{{$t('content.safety.hse.list.column.첨부파일')}}</span></vs-th>
            <vs-th ><vs-col vs-type="flex" vs-justify="center">{{$t('content.safety.hse.list.column.보기')}}</vs-col></vs-th>
          </template>
          ...
          <template slot-scope="{data}">
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td class="text-center">{{ getHseTypeSafety(getContent(tr).hse_type_safety) }}</vs-td>
...
export default {
  ...
  methods: {
    ...
    async loadPostList () {
      ...
      keyword.push('content$preview.hse_type*safety')
    ...
    getHseTypeSafety (type) {
      const selectSafetyList = {
        'regular_safety': '정기안전교육',
        'hiring_safety': '채용시 안전교육',
        'changing_safety': '작업내용 변경시 안전교육',
        'special_safety': '특별안전교육',
        'supervisor_safety': '관리감독자 안전교육',
        'material_safety': 'MSDS',
        'etc_safety': '기타(안전)',
      }
    ...
    handleClickRegister () {
      this.$emit('handleClickRegister', 'safety')
    },

    handleItemClickDetail (data) {

      this.$router.push(`/project/${data.project_id}/hse/list/safety/${data.id}`)
    },
    handleItemClickDetailBlank (data) {
      const routeData = this.$router.resolve(`/project/${data.project_id}/hse/list/safety/${data.id}`)
