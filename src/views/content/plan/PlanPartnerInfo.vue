<template>
  <div>
    <search-nav @handleSearch="handleSearch">
      <search-nav-item-project class="w-full" size="large" :label="$t('search.현장명')" v-model="nav.projectId" @select="handleSearch"/>
    </search-nav>
      ...
      <vs-row class="mb-8">
        ...
        <vs-col class="w-5/12 grid" style="position: relative">
          ...
          <div style="position: absolute; top:-8px; right:0px;">
            <vs-button v-if="selectedCompany" class="small" color="primary" type="filled" @click.stop="onClickUpdate">프로젝트에서 편집</vs-button>
          </div>
        </vs-col>
      </vs-row>
      ...
      <vs-row>
        ...
        <vs-col class="w-full">
          <vs-row class="mb-4 flex" vs-align="center">
            <h5 class=""><i class="dot"></i>사용자정보</h5>
...
export default {
  ...
  methods:{
    ...
    handleSearch () {
      this.$refs.tablePlanPartnersInfo.loadList()
    ...
    onClickDetail (value) {
      ...
      this.nav.projectId = value.projId
      this.loadMembers()
    },
    ...
    onClickUpdate () {
      const proj_id = `${this.nav.projectId}`
      this.$router.push({name:'project-plan-partners', params:{proj_id}})
    },
    getCol (index, value) {
      switch (index) {
