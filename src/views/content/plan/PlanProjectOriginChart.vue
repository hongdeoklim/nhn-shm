...
export default {
  ...
  data () {
    return {
      ...
      projectTitle: '',
      column : [
        this.$t('content.plan.project.origin-chart.column.No'),
        this.$t('content.plan.project.origin-chart.column.현장명'),
        this.$t('content.plan.project.origin-chart.column.지역'),
        this.$t('content.plan.project.origin-chart.column.안전보건부서장'),
        this.$t('안전보건담당자'),
        this.$t('content.plan.project.origin-chart.column.보기')
      ],
      list:[],
  ...
  computed: {
    projectList () {
      ...
      for (let i = 0; i < this.list.length; i++) {
        ...
        newList.push([no, field_name, field_location, field_chief, safety_chief])
      }
