<template>
  <div>
    ...
    <search-nav @handleSearch="handleSearch">
      ...
      <search-nav-item-keyword :label="$t('search.파트너사')" v-model="nav.company" />
    ...
    <vs-row>
      <vs-col>
        ...
        >
          ...
          <template slot="thead">
            <vs-th sort-key="id" class="w-1/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.No')}}</span></vs-th>
            <vs-th sort-key="post_title" class="w-2/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.현장명')}}</span></vs-th>
            <vs-th sort-key="content$preview.work" class="w-2/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.교육명')}}</span></vs-th>
            <vs-th sort-key="content$preview.target" class="w-1/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.교육대상')}}</span></vs-th>
            <vs-th sort-key="content$preview.main_manager" class="w-2/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.교육담당')}}</span></vs-th>
            <vs-th sort-key="content$preview.date" class="w-2/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.교육일')}}</span></vs-th>
            <vs-th sort-key="content$preview.total" class="w-1/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.총인원')}}</span></vs-th>
            <vs-th class="w-1/12" ><span class="w-full text-center">{{$t('content.state.hse.list.column.첨부파일')}}</span></vs-th>
            <vs-th style="width: 200px !important; display: inline-block;"><vs-col vs-type="flex" vs-justify="center">{{$t('content.state.hse.list.column.보기')}}</vs-col></vs-th>
          </template>
...
export default {
  ...
  methods: {
    ...
    async loadPostList () {
      ...
      keyword.push('content$preview.hse_type*environ')
    ...
    handleClickRegister () {
      this.$emit('handleClickRegister', 'environ')
    },

    handleItemClickDetail (data) {
      this.$router.push({path: `/project/${data.project_id}/hse/list/environ/${data.id}`})
    },
    handleItemClickDetailBlank (data) {
      if (data) {
        const routeData = this.$router.resolve(`/project/${data.project_id}/hse/list/environ/${data.id}`)
