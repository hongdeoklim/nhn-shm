<template>
  <div>
    <search-nav @handleSearch="handleSearch">
      <search-nav-item-keyword class="w-full" :label=" $t('search.검색')" v-model="nav.keyword"></search-nav-item-keyword>
    </search-nav>
    ...
    <vs-card >
      <vs-table
        ...
        :data="table.list"
        noDataText="결재문서가 없습니다."
        @selected="handleItemClickDetail">

        <template slot="thead">
          <vs-th style="min-width:130px;"><span class="w-full text-center">{{ $t('approval.No') }}</span></vs-th>
...
export default {
  ...
  methods: {
    ...
    async loadPostList () {
      ...
      await this.$store.dispatch('approval/LOAD_REFERENCE_LIST',{page, perPage, boardSlug, projectId})
